...
//        self.test();
    ...
    this.updateTrainees = function(list){
        ...
        for (var i in list){
            if (map[list[i].id] == undefined){
                ...
                map[list[i].id].lastSessionEndTimestamp = list[i].lastSessionEndTimestamp;
            }else{
                map[list[i].id].bpm = (list[i].bpm == undefined) ? 0 : Math.round(list[i].bpm);
                map[list[i].id].SDNN = Math.floor(list[i].SDNN * 10.0) / 10.0;
            }
    ...
    this.translateRu = function(){
        $('.translatable').each(function(){
            var t = $(this).attr('data-trRu');
            $(this).text(t);
        });
    }
...
CardioMoodTrainee = function(traineeId){
    ...
    this.lastName = '';
    this.lastIntervals = [];
    this.lastSessionEndTimestamp = undefined;
    ...
    this.getMonitoringBlockHTML = function(num){
        ...
        s+= '<div class="col-sm-4 ' + ((self.lastSessionEndTimestamp != undefined) ? 'notActive' : '') +'">'+
            '<div data-id="' + self.id +'" class="trainee-tier tile-progress  ' + styles[num % styles.length] +'">' +

            '<div class="tile-header">' +
            '<h3>' +self.firstName + ' ' + (self.lastName == undefined ? '' : self.lastName) + '</h3>' +
