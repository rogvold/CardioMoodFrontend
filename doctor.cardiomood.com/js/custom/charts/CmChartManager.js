...
var CmChartManager = function(){
    ...
    this.calculatedParamsMap = {};
    this.recurGeneratedParamsMap = {};
...
//        $('.params_table_placeholder').hide();
    ...
    this.init = function(){
        self.initRRPlots();
        //console.log('initRRPlots - success');
        self.initRadios();
        //console.log('initRadios - success');
        self.initNavButtons();
        //console.log('initNavButtons - success');
        self.initTableHead();
        //console.log('initTableHead - success');
        self.initExportButton();
        //console.log('initExportButton - success');
        self.initFullReportBlock();
        //console.log('initFullReportBlock - success');
    }

    this.initRRPlots = function(){
        console.log('initRRPlots - trying');
        self.mainRRChart = new LineChart();
        self.mainRRChart.selectionCallback = self.selectedPointsCallback;
        console.log('mainRRChart - try to init', self.mainRRChartDivId);
...
//        console.log(points);
//        self.mainRRChart.startTimestamp = self.startTimestamp;
    ...
    this.calculateSpectrum = function(){
        ...
        $.ajax({
            ...
            url: 'http://calc.cardiomood.com/CardioMoodCalc/webresources/calc/getACF',
            type: 'POST',
            data: {
                data: JSON.stringify({
                    series: series
...
//        $('#params_block').html(s);
    ...
    this.initExportButton = function(){
        $('#reportDownloadLink').bind('click', function(){
            var name = 'session';
            var date = moment().format('LLLL');
            var header = [];
            var data = [];
            data.push([]);
            var map = self.calculatedParamsMap;
            for (var key in map){
                header.push(key);
                data[0].push(map[key]);
            }
            ...
            console.log(ds);
            post('http://5.9.107.99:5014/getXLS', {data: ds}, 'post');
        });
    }
    ...
    this.selectWindow = function(from, duration){
        ...
        self.selectedRRChart.startTimestamp = self.startTimestamp;
...
//        $('#calc_params_placeholder').html(s);
    ...
    this.initFullReportBlock = function(){
        $('#fullReportLink').bind('click', function(){
            self.prepareRecurCalculatedMap();
            var d = self.recurGeneratedParamsMap;
            var ds = JSON.stringify(d);
            post('http://5.9.107.99:5014/getXLS', {data: ds}, 'post');
        });
    }

    this.prepareRecurCalculatedMap = function(){
        var fact = self.generationParamsFactory;
        var header = [];
        var data = [];
//        for (var f in fact){
        var list = fact["120000"];
        if (list.length == 0){
            return;
        }
        ...
        self.recurGeneratedParamsMap = d;
...
//                self.recurGenerating(cur + window, window, finishCallback);
    ...
    this.loadSportsParams = function(times, rrs, callback){
        var d = {
            time: times,
            series: rrs
        }
        $.ajax({
            //url: 'http://calc.cardiomood.com/CardioMoodCalc/webresources/calc/getSpectrum',
            url: 'http://calc.cardiomood.com/CardioMoodCalc/webresources/calc/getSportsParamsPlotData',
            type: 'POST',
            data: {
